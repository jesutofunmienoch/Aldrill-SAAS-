...
export const getBookmarkedCompanions = async (userId: string) => {
  ...
  // Step 1: Get all companion_ids from bookmarks for the user
  ...
  // Step 2: Fetch companions manually
  const { data: companions, error: companionError } = await supabase
    .from("companions")
    .select("*")
    .in("id", companionIds); // 👈 this returns Companion[]

  if (companionError) throw new Error(companionError.message);

  return companions; // ✅ companions is already Companion[]
};
